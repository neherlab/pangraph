...
fixed policy. Natural settings, chosen by a CLI flag: **all** (`M == N`, only unanimous placements),
**a fraction** (`M >= ceil(f · N)` for a user fraction `f`), or **any** (`M == 1`, every distinct
placement). Exact-coordinate equality is the baseline identity test; whether to allow a small
coordinate **tolerance** (to absorb a terminus nudged by a nearby indel) is an open refinement. The
exact flag surface and fraction semantics are still **to be decided** — the P5.1 node-level prototype
on real data is meant to inform this choice (see §10).
