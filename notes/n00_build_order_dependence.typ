...
- *`-X` discards real signal.* The two directions find measurably different homology (147 vs 146
  hits). Currently one is chosen arbitrarily. This is a quality concern in its own right, pursued
  in the dual-mapping alternative section.
...
This diagnosis has a direct consequence: if dual mapping is what protected mmseqs, then enabling it
for minimap2 should protect minimap2 too, without any canonical naming. That prediction was tested,
and it holds --- see the dual-mapping alternative section.
...
== The dual-mapping alternative

The finding that mmseqs was never affected raises an obvious question: if computing both directions
is what made mmseqs immune, could minimap2 simply be told to do the same --- fixing the defect
without any of the naming machinery? The answer is yes, with caveats that decide whether it is the
right trade.

=== Disabling `-X` outright does not work

`X` is a bundle, not a toggle. In the in-tree wrapper (`packages/minimap2/src/options_args.rs:324`)
a single boolean sets four flags:
...
and the `asm5`/`asm10`/`asm20` presets pangraph uses set none of them (unlike the `ava-*` presets,
`packages/minimap2-sys/minimap2/options.c:96`). So `X: false` also discards all-chains and
no-diagonal reporting. Measured on the three *Campylobacter* genomes, the result is order-invariant
and useless:
...
#table(
  ...
  table.hline(),
  [`X: false`], [3], [yes],
  [`-X` with only `MM_F_NO_DUAL` cleared], [459], [yes],
)

Three blocks for three genomes means nothing merged at all. The likely mechanism --- inferred from
minimap2's secondary-alignment filtering rather than instrumented --- is that without `NO_DIAG` each
sequence's perfect self-hit becomes the primary chain, and cross-genome hits are then discarded as
low-ratio secondaries.

=== Enabling dual mappings does work

The precise variant is minimap2's `-X --dual=yes`: keep all-chains, no-diagonal and no-long-join,
clear only `MM_F_NO_DUAL`. Tested against the #emph[entire pre-fix codebase] --- `BlockId` sequence
names, `BlockId` ordering, reference-wins-ties anchor selection, input-order neighbor-joining leaves
--- all six argument permutations produced 459 blocks with an identical consensus-set digest
(`e33506c21d945b73`). No canonical naming, no lookup table, no tie-break changes.
...
ties, and reference-wins-ties would still privilege one side. Mirror alignments happen to score
slightly differently, so no tie arises in practice --- which is exactly why mmseqs looked immune.
#emph[Dual mappings make invariance a property of the data; canonical naming makes it a property of
the code.] If two mirror alignments ever scored equal, the order dependence would return.
