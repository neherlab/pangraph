...
mod tests {
  ...
  use pangraph::io::genbank::GenbankReader;
  ...
  //                     path name (bare)   gff path                                     genbank path
  const KLEBS: &[(&str, &str, &str)] = &[
    (
      "NZ_CP013711",
      "../../data/klebs_annotations/NZ_CP013711.gff.gz",
      "../../data/klebs_annotations/NZ_CP013711.gbk.gz",
    ),
    (
      "NC_017540",
      "../../data/klebs_annotations/NC_017540.gff.gz",
      "../../data/klebs_annotations/NC_017540.gbk.gz",
    ),
  ];
  ...
  #[case(0)]
  #[case(1)]
  fn smoke_parse_real_klebs_genbank(#[case] i: usize) -> Result<(), Report> {
    let (_, _, gbk) = KLEBS[i];
    let features = GenbankReader::from_path(gbk)?.read_many()?;
    assert!(
      features.len() > 1000,
      "expected thousands of features, got {}",
      features.len()
    ...
    assert!(features.iter().any(|f| f.feature_type == "CDS"));
    assert!(features.iter().any(|f| f.strand == Some(Strand::Reverse)));
    assert!(features.iter().any(|f| f.id.is_some()));
    Ok(())
  }
