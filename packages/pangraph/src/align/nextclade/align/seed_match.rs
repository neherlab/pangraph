...
impl SeedMatch2 {
  fn extend_seed<L: Letter<L>>(&self, qry_seq: &[L], ref_seq: &[L], config: &NextalignParams) -> Vec<SeedMatch2> {
    let SeedMatch2 {
      mut ref_pos,
      mut qry_pos,
      mut length,
...
impl CodonSpacedIndex {
  ...
  fn index_matches(&self, qry_seq: &[Nuc], config: &NextalignParams) -> Vec<SeedMatch2> {
  ...
  fn extended_matches(&self, qry_seq: &[Nuc], ref_seq: &[Nuc], config: &NextalignParams) -> Vec<SeedMatch2> {
