...
convert = {
    ...
    "n-isolates": lambda x: len(
        [y for y in x.split(" ") if y.startswith("panx_results/pangraphs/")]
    ),
    ...
    "command": lambda x: " ".join(
        [y for y in x.split(" ") if not y.startswith("panx_results/pangraphs/")]
    ),
}
...
def plot_benchmark(df):
    ...
    order = order_kinds(list(df["kind"].unique()))
    df["kind"] = pd.Categorical(df["kind"], categories=order, ordered=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="creates a table with summary statistics from the benchmark data"
    )
    ...
    parser.add_argument(
        "benchmarks", nargs="*", help="list of input summary txt files", type=str
    )
